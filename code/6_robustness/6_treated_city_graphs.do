...
preserve
    ...
    foreach cbsa in /*47460 16820 20500 39580 38900 41860 33460 24340 16740 44060 40340*/ 12060 47900 26980 14260{
        ...
        if `cbsa' == 41860{
            local treat_year = 2022
            local city = "San Francisco"
        }
        if `cbsa' == 33460{
            local treat_year = 2020
        ...
        twoway (line total year, color("`forest_green'")) ///
               (line one_unit year, color("`brown'")) ///
               (line two_unit year, color("`peru'")) ///
               (line three_four_unit year, color("`gray'")) ///
               (line five_plus_unit year, color("`blue'")) ///
               if cbsa == `cbsa' & year < 2024, xtitle("Year") ytitle("Permits Issued") xlabel( , nogrid) ylabel( , nogrid) xline(`treat_year') ///
		       legend(order(1 "Total Permits" 2 "One-Unit" 3 "Two-Unit" 4 "Three- and Four-Unit" 5 "Five-Plus Unit")) title("`city'")
        graph export "${figures}/`city'_permits_over_time.png", replace
    }
restore
